...
final class AppCatalogUpdater {
    ...
    void applyRequest(SaveAppRequest request, AppEntity agent) {
        ...
        agent.setMode(AppMode.from(requestedOrCurrentOrDefault(
                request.getMode(), agent.getMode(), AppMode.AGENT.value())).value());
