...
public class OpenClawController {
    ...
    public ApiResponse<OpenClawDtos.PluginInfo> install(@RequestBody OpenClawDtos.InstallRequest request) {
        OpenClawDtos.PluginInfo installed = client.install(request); refresh(); return ApiResponse.ok(installed);
    }
    ...
    public ApiResponse<Void> enable(@PathVariable String pluginId) { client.enable(pluginId); refresh(); return ApiResponse.ok(null); }
    @PostMapping("/plugins/{pluginId}/disable")
    public ApiResponse<Void> disable(@PathVariable String pluginId) { client.disable(pluginId); refresh(); return ApiResponse.ok(null); }
    @DeleteMapping("/plugins/{pluginId}")
    public ApiResponse<Void> uninstall(@PathVariable String pluginId) { client.uninstall(pluginId); refresh(); return ApiResponse.ok(null); }
