-- ============================================================================
-- Migration V1.3 · Split goodle_apps.graph_json into a proper goodle_workflows.app_id draft
--                  (Dify parity — one app → many goodle_workflows, one mutable draft)
-- ----------------------------------------------------------------------------
-- Before this migration:
--   * `goodle_apps.graph_json` inlined the visual-designer DAG on the app row, so
--     GET /agents returned the whole graph on every list call.
--   * `goodle_workflows` was a flat table with no link back to the owning app, no
...
--
-- After:
--   * `goodle_apps.workflow_id` is a FK to `goodle_workflows.id` — points to the DRAFT for
--     workflow/chatflow modes (by convention `goodle_workflows.id == goodle_apps.id`), and
--     gets rebound to the latest published snapshot after each publish.
--   * `goodle_workflows.app_id` is the reverse FK. `version` becomes VARCHAR — 'draft'
--     for the mutable working copy, timestamp/label for immutable snapshots.
--   * `goodle_workflows` gains `features` / `environment_variables` /
...
-- data to preserve, hand-materialize it into goodle_workflows before running:
--
--   INSERT INTO goodle_workflows (id, tenant_id, app_id, name, mode, graph_json,
...
--   UPDATE goodle_apps SET workflow_id = id
--    WHERE mode IN ('workflow', 'chatflow') AND graph_json IS NOT NULL;

ALTER TABLE goodle_apps ADD COLUMN IF NOT EXISTS workflow_id VARCHAR(64);
ALTER TABLE goodle_apps DROP COLUMN IF EXISTS graph_json;

CREATE INDEX IF NOT EXISTS idx_apps_workflow ON goodle_apps (workflow_id);
...
ALTER TABLE goodle_workflows ADD COLUMN IF NOT EXISTS app_id                 VARCHAR(64);
ALTER TABLE goodle_workflows ADD COLUMN IF NOT EXISTS features               TEXT;
ALTER TABLE goodle_workflows ADD COLUMN IF NOT EXISTS environment_variables  TEXT;
ALTER TABLE goodle_workflows ADD COLUMN IF NOT EXISTS conversation_variables TEXT;
ALTER TABLE goodle_workflows ADD COLUMN IF NOT EXISTS output                 TEXT;
ALTER TABLE goodle_workflows ADD COLUMN IF NOT EXISTS marked_name            VARCHAR(255);
ALTER TABLE goodle_workflows ADD COLUMN IF NOT EXISTS marked_comment         VARCHAR(1024);
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
         WHERE table_name = 'goodle_workflows' AND column_name = 'version'
           AND data_type = 'integer'
    ) THEN
        ALTER TABLE goodle_workflows
...
ALTER TABLE goodle_workflows ALTER COLUMN published SET DEFAULT FALSE;
...
ALTER TABLE goodle_workflows ALTER COLUMN name DROP NOT NULL;
