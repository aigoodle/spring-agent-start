-- ============================================================================
-- Migration V1.1 · Add tenant_id column to goodle_predefined_model
-- ----------------------------------------------------------------------------
-- Problem: PredefinedModelEntity extends BaseEntity which declares tenant_id.
-- The initial `goodle_predefined_model` DDL omitted this column because the
...
ALTER TABLE goodle_predefined_model
...
UPDATE goodle_predefined_model SET tenant_id = 'system' WHERE tenant_id IS NULL OR tenant_id = '';

CREATE INDEX IF NOT EXISTS idx_predef_tenant_provider
    ON goodle_predefined_model (tenant_id, provider_name);
