-- ============================================================================
-- Migration V1.6 · goodle_apps.model_name + goodle_apps.model_provider columns
-- ----------------------------------------------------------------------------
-- Replaces the ambiguous goodle_apps.model_id column with two explicit ones:
...
-- goodle_model.id UUID, so nothing downstream could actually resolve the value.
...
--
-- Copies any existing model_id value into model_name so previously-saved goodle_apps
-- keep working. The legacy column is dropped once the copy succeeds.
-- ============================================================================

BEGIN;

ALTER TABLE goodle_apps ADD COLUMN IF NOT EXISTS model_name     VARCHAR(128);
ALTER TABLE goodle_apps ADD COLUMN IF NOT EXISTS model_provider VARCHAR(64);
...
ALTER TABLE goodle_apps DROP COLUMN IF EXISTS model_id;
